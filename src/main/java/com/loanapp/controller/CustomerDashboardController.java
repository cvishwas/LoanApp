...
public class CustomerDashboardController {
	...
	public String customerDashboard(Model model, HttpServletRequest request) {
		...
		// get the customerId
		int customerId = (Integer) jdbcTemplate.queryForObject(
				"select CUSTOMER_INFO from LOAN_APP_USERS where USER_LOGIN = '" + username + "'", Integer.class);

		// get the customers loans
		List<Loan> loans = jdbcTemplate.query("select * from LOAN_APP_LOANS where CUSTOMER_ID = " + customerId,
				new RowMapper<Loan>() {
		...
		List<String> types = jdbcTemplate.query("select LOAN_TYPE from LOAN_APP_LOAN_TYPES order by LOAN_ID",
